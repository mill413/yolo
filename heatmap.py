    ...
    def process(self, img_path, save_path):
        ...
        if self.show_box:
            for data in pred:
                ...
                ))], f'{self.model_names[int(data[4:].argmax())]} {float(data[4:].max()):.2f}', cam_image)
    ...
    def __call__(self, img_path, save_path):
        ...
        if os.path.isdir(img_path):
            for img_path_ in os.listdir(img_path):
                self.process(f'{img_path}/{img_path_}',
                             f'{save_path}/{img_path_}')
        else:
            self.process(img_path, f'{save_path}/result.png')


def parse_arguments():
    parser = argparse.ArgumentParser(description="Heatmap parse arguments.")

    parser.add_argument("--model", type=str, dest="model",
                        help="trained pt model file in models/ dir")
    ...
    return (model_name, source, dataset)


if __name__ == '__main__':
    model_name, source, dataset = parse_arguments()
    params = {
        "weight": f"runs/{model_name}/{dataset}/train/weights/best.pt",
        "device": "cuda:0",
